// src/components/blog/author/AuthorPostsList.tsx
'use client'
import Link from 'next/link'
import { Post } from '@/payload-types'
import { useState } from 'react'
import { fetchPostsByAuthor } from '@/actions/fetchPostsByAuthor'
...
export default function AuthorPostsList({
  ...
  async function goToPage(targetPage: number, type: 'prev' | 'next') {
    try {
      ...
      const res = await fetchPostsByAuthor(authorName, targetPage)
  ...
  return (
    <article className="space-y-6 w-full">
      ...
      <nav className="flex items-center gap-2 text-sm text-gray-600 mb-6">
        ...
        <span>→</span>
        <Link href="/blog/author" className="hover:text-blue-600 transition-colors">
          Authors
        </Link>
        <span>→</span>
      ...
      {currentPosts.length === 0 && (
        <div className="text-center py-12">
          <p className="text-gray-600 mb-4">No posts found by &quot;{authorName}&quot;.</p>
          <Link
            href="/blog/author"
